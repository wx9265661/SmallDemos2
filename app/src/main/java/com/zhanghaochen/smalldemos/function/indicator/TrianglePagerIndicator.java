 ...
 */
package com.zhanghaochen.smalldemos.function.indicator;
...
import com.zhanghaochen.smalldemos.function.indicator.abs.IPagerIndicator;
import com.zhanghaochen.smalldemos.function.indicator.support.ArgbEvaluatorHolder;
import com.zhanghaochen.smalldemos.function.indicator.support.PositionData;
import com.zhanghaochen.smalldemos.utils.SysUtils;

import java.util.List;

/**
 * 三角形viewpager指示器，带颜色渐变
 */
public class TrianglePagerIndicator extends View implements IPagerIndicator {
    ...
    public static final int MODE_EXACTLY = 2;  // 直线宽度 == mTriangleWidth
    ...
    private void init() {
        ...
        mTriangleHeight = SysUtils.convertDpToPixel(5);
        mTriangleWidth = SysUtils.convertDpToPixel(10);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        mPath.reset();
        mPath.moveTo(mLineRect.left, mLineRect.bottom);
        mPath.lineTo(mLineRect.right, mLineRect.bottom);
        mPath.lineTo((mLineRect.left + mLineRect.right) / 2, mLineRect.top);
        mPath.close();
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mMode == MODE_MATCH_EDGE) {
            ...
            leftX = current.mLeft + (current.width() - mTriangleWidth) / 2;
            nextLeftX = next.mLeft + (next.width() - mTriangleWidth) / 2;
            rightX = current.mLeft + (current.width() + mTriangleWidth) / 2;
            nextRightX = next.mLeft + (next.width() + mTriangleWidth) / 2;
        }
        ...
        mLineRect.top = getHeight() - mTriangleHeight - mYOffset;
    ...
    public void setTriangleWidth(float width) {
